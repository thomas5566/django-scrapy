import scrapy
from crawling.crawling.items import MovieItem
...
class RottentomatoesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        rows = response.xpath("//*[@class='table']/tbody/tr/td[3]/a/@href").extract()

        for row in rows:
            link = 'https://www.rottentomatoes.com' + row
            yield {
                scrapy.Request(url=link, callback=self.parse_item)
            }
    ...
    def parse_item(self, response):
        ...
        i['amount_reviews'] = response.css('.mop-ratings-wrap__text--small ::text').ectract()[1]
